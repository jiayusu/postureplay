/**
 * useSimulation — GPU 仿真管线 Hook (v2)
 *
 * 管理完整 GPU 仿真管线的生命周期：
...
export function useSimulation(
  ...
  // ── 核心渲染步进 ──
  const stepSimulation = useCallback((_now: number) => {
  ...
  useEffect(() => {
    ...
    if (!canvas) { console.warn('[Sim] ⚠️ canvas is null, bailing'); return }
  ...
  return {
    setConfig: (update: Partial<SimulationConfig>) => {
      Object.assign(cfg.current, update)
    },
    step: () => stepSimulation(performance.now()),
  }
}
